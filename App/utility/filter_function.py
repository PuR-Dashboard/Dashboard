...
def filter_max_value(df: pd.DataFrame, category:str, max_value:int) -> pd.DataFrame:
    ...
    #filtering process
    try:
        df2 = df.drop(df.loc[df[category] > max_value].index)#dropping all rows of the datframe which value for the given category is higher than the max_value
    except Exception as e:
        raise e("Something went wrong while filtering for a maximum value!")#error message if the filter did not work
...
def filter_for_value(df:pd.DataFrame, category:str, set_value:str) -> pd.DataFrame:
    ...
    #filtering process
    try:
        df2 = df.drop(df.loc[df[category] != set_value].index)#dropping all rows of the datframe which value for the given category does not euqal the given value
    except Exception as e:
        raise e("Something went wrong when filtering for a value!")#error message if the filter did not work
...
def filter_for_list(df:pd.DataFrame, category:str, set_list:list,  filter_occupancy=False) -> pd.DataFrame:
    ...
    try:
        ...
        raise e("Something went wrong when filtering for a list of values!")#error message if the filter did not work
...
def get_occupancy_list_from_vals(occupancy_vals:list[str]) -> list[str]:
    ...
    #if no list with occupancies are given
   ...
   #iterate over all characteristics/keys of the data
    ...
    for key in filter_dict:
        ...
        #occupancy values have to be preprocessed, and filtering happens on location names based on that, hence own if statement
            ...
            if type(oc) != list:#we need the occupancies in as list type
                oc = [oc]

            df = filter_for_list(df, "location", get_occupancy_list_from_vals(oc), filter_occupancy=True)


        #if single string is given, only filter for that value
